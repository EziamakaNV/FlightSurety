...
contract('Flight Surety Tests', async (accounts) => {
  ...
  before('setup contract', async () => {
    ...
    //console.log('Surety App Address: ----> ',config.flightSuretyApp.address);
    ////console.log('Is surety app authorized: ', await config.flightSuretyData.isAuthorized.call(config.flightSuretyApp.address));
    try {
      //console.log("Authorizig Surety App Contract ..........");
  ...
  it('(airline) airline needs to be funded with 10 ether', async () => {
    const AIRLINE_FUNDING_VALUE_LOWER = web3.utils.toWei("8", "ether");
  ...
  it('(airline) airline can participate in contract when it is funded', async () => {
    try {
      //console.log(`flsuretyappaddress: ${config.flightSuretyApp.address}`);
      //console.log(`first airline addy: ${config.firstAirlineAddress}`);
